...
import { migratePlannerStoreV8ToV9 } from "../server/store/sqlite-store.ts";
...
function parseMigrationCommand(arguments_: readonly string[]): MigrationCommand {
  if (arguments_.length !== 4) {
    throw new TypeError("Usage: --database /absolute/planner.sqlite --backup /absolute/planner.pre-v9.sqlite");
  }
...
function runMigrationCommand(arguments_: readonly string[]): string {
  ...
  return JSON.stringify(migratePlannerStoreV8ToV9({
