...
4. **Groceries contain purchase requirements, not every named material.** Prepared components, outputs, finished dishes, leftovers, and reserved portions do not become Shop rows merely because a step names them. Source: audited live Groceries behavior and `docs/functional-spine.md`, ING-4.
...
2. **Planner domain authority** owns household concepts, accepted vocabulary, occurrence resolution, grocery eligibility, source/check execution state, version checks, events, undo, and atomic batch apply.
...
**Scope:** Household concept records and vocabulary; stable occurrence links; unresolved/resolved/renamed/merged lifecycle; curated-catalogue bootstrap decision; single and bounded-list preview; explainable candidate results with edit distance as one signal; version-bound atomic apply; partial measurement normalization; grocery eligibility; safe migration preserving literals, IDs, instruction links, and grocery execution state; removal of duplicated semantic ownership.
...
**Scope:** Extract the touched recipe/step ingredient editors and presentation from `planner-client.tsx`; retain text-first editing; offer candidate and batch exception review without forcing resolution; preserve the existing instruction-step interaction; fix recipe-drawer ingredient layout/overflow; make recipe provenance clear where Prep ingredient/step context otherwise becomes ambiguous; reuse the existing canonical read components across Recipe, Day, and Prep.
...
**Scope:** Project only grocery-eligible occurrences; build a grouped read model without creating group-owned durable state; show compatible totals and visible literal requirements such as `1 bunch + 1/3 cup sliced`; retain contributing meals and stable child identities; correct source/filter behavior including Farm box/On hand; route edits back to recipe occurrence or grocery-owned classification; extract the touched grocery controls/view from `planner-client.tsx`.
...
1. Whether the initial household catalogue is a curated starter set, a catalogue derived from existing meal data, or a hybrid.
...
3. Whether existing ingredients migrate as grocery-eligible by default and are reviewed, or migration attempts an automatic initial exclusion of prepared outputs.
