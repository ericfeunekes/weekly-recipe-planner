...
The host owns planner identity, authorization, idempotency, persistence, and every durable planner effect. Use planner.read for canonical state, planner.preview for a pure check, planner.apply for an atomic operation batch, and planner.importRecipe to pin one reviewed canonical file from the configured recipe root into an eligible unstarted meal. Never request direct filesystem access to recipes. A planner change succeeded only when the host reports an accepted durable result. After a version conflict, read again before making a new call. The planner tool schema is authoritative.
