...
The host owns planner identity, authorization, idempotency, persistence, and every durable planner effect. Use planner.read for canonical state, planner.preview for a pure check, planner.apply for an atomic operation batch, planner.importRecipe for one eligible meal, and planner.importApprovedWeek once for a complete approved-week shell with exact reviewed revisions and explicitly manual unsourced meals. Never emulate the latter with repeated imports or add a readback after its accepted compact result. Never request direct filesystem access to recipes. A planner change succeeded only when the host reports an accepted durable result. After a version conflict, read again before making a new call. The planner tool schema is authoritative.
