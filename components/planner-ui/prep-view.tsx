                ...
                if (isPrepSessionCombinedStep(entry)) {
                  ...
                  return <article key={entry.id} className={`instruction-step instruction-step-line prep-queue-step combined-prep-step${entry.complete ? " complete" : ""}${selected ? " selected" : ""}${entry.needsReview ? " needs-review" : ""}`} data-testid="prep-combined-step" data-prep-date={selectedPrepDate} data-prep-queue-date={selectedPrepDate} data-prep-queue-position={index} draggable={!disabled} onMouseDown={(event) => { if (event.button !== 0 || isPrepRowControlTarget(event.target)) return; beginPointerDrag({ kind: "session", sourcePrepDate: selectedPrepDate, entryIds: draggedEntryIds }, event); selectSessionEntry(entry.id, event); }} onDragStart={(event) => { if (disabled || isPrepRowControlTarget(event.target)) return; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-prep-date-entries", JSON.stringify(draggedEntryIds)); startSessionDrag(draggedEntryIds); }} onDragEnd={endDrag} onDragOver={(event) => { if (!isPrepDrag(event)) return; event.preventDefault(); setDropTargetPrepDate(selectedPrepDate); setDropInsertion({ prepDate: selectedPrepDate, position: event.clientY < event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2 ? index : index + 1 }); }} onDrop={(event) => receivePrepDrop(event, selectedPrepDate, event.clientY < event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2 ? index : index + 1)}>
                    <div className="instruction-line-main prep-queue-main combined-prep-main">
                      ...
                      <div className="combined-prep-content">
                        <div className="combined-prep-heading"><strong>{entry.instruction}</strong><span>{entry.needsReview ? "Needs review" : "Combined prep"}</span></div>
                        {projection.aggregates.length ? <div className="combined-prep-aggregates">{projection.aggregates.map((aggregate) => <span key={aggregate.key}>{aggregate.display}</span>)}</div> : null}
                        <div className="combined-prep-sources" aria-label="Source recipe steps">
                          {projection.sources.map((source) => {
                            ...
                            return <div className="combined-prep-source" key={source.stepId}><div><strong>{dateLabel} · {source.mealTitle}</strong><span className="combined-prep-source-step">{source.instruction}</span></div>{contribution ? <span>contributes {contribution}</span> : null}</div>;
                          })}
                        </div>
                        <p className="combined-prep-effect">{entry.needsReview ? "A source recipe changed. Review this wording before completing the batch." : entry.complete ? `Prepared in batch for ${recipeCount} ${recipeCount === 1 ? "recipe" : "recipes"}; dinner steps remain unchecked.` : `Completing this prep task prepares work for ${recipeCount} ${recipeCount === 1 ? "recipe" : "recipes"}; dinner steps stay unchecked.`}</p>
                      </div>
                      <details className="prep-overflow combined-prep-overflow" data-prep-row-control>
                        <summary className="planner-icon-button prep-overflow-trigger" aria-label={`Options for combined prep batch ${entry.instruction}`}><MoreVertical size={16} /></summary>
                        <div className="prep-overflow-menu" aria-label={`Actions for combined prep batch ${entry.instruction}`}>
                          <label className="combined-prep-menu-select"><input type="checkbox" checked={selected} disabled={disabled} aria-label={`Select combined prep batch ${entry.instruction}`} onChange={(event) => toggleSessionEntry(entry.id, event.target.checked)} /> Select batch</label>
                    ...
                    {editing ? <div className="instruction-inline-comment combined-prep-editor"><textarea aria-label="Combined prep instruction" value={combinedInstruction} onChange={(event) => setCombinedInstruction(event.target.value)} /><div className="step-comment-actions"><PlannerActionButton tone="secondary" type="button" onClick={() => setEditEntryId(null)}>Cancel</PlannerActionButton><PlannerActionButton tone="secondary" type="button" disabled={disabled || !combinedInstruction.trim()} onClick={() => entry.complete ? previewDiscard({ type: "updateCombinedPrepStep", weekId: week.id, entryId: entry.id, instruction: combinedInstruction.trim(), discardFulfillment: true }, "close-edit") : void mutate({ type: "updateCombinedPrepStep", weekId: week.id, entryId: entry.id, instruction: combinedInstruction.trim() }, { onAccepted: () => setEditEntryId(null) })}>Save batch</PlannerActionButton></div></div> : null}
                  </article>;
                }
