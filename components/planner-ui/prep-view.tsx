...
import { CheckCircle2, GripVertical, MoreVertical, Plus, Trash2, X } from "lucide-react";
...
function closePrepDisclosure(target: HTMLElement): void {
            ...
            <div className="prep-step-list">
              {selectedSession?.steps.map((entry, index) => {
                if (isPrepSessionCombinedStep(entry)) {
                  ...
                  const recipeCount = new Set(projection.sources.map((source) => source.mealId)).size;
                  return <article key={entry.id} className={cn("instruction-step instruction-step-line prep-queue-step border-l-[3px]! border-l-[color-mix(in_srgb,var(--green)_42%,var(--line))]! py-4! pr-0! pl-4! max-[840px]:pl-2.5!", entry.complete && "complete bg-[color-mix(in_srgb,var(--green-soft)_35%,transparent)]!", selected && "selected bg-[var(--green-soft)]! shadow-[inset_3px_0_0_var(--green)]!", entry.needsReview && "needs-review border-l-[var(--amber)]!")} data-testid="prep-combined-step" data-prep-date={selectedPrepDate} data-prep-queue-date={selectedPrepDate} data-prep-queue-position={index} draggable={!disabled} onMouseDown={(event) => { if (event.button !== 0 || isPrepRowControlTarget(event.target)) return; beginPointerDrag({ kind: "session", sourcePrepDate: selectedPrepDate, entryIds: draggedEntryIds }, event); selectSessionEntry(entry.id, event); }} onDragStart={(event) => { if (disabled || isPrepRowControlTarget(event.target)) return; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-prep-date-entries", JSON.stringify(draggedEntryIds)); startSessionDrag(draggedEntryIds); }} onDragEnd={endDrag} onDragOver={(event) => { if (!isPrepDrag(event)) return; event.preventDefault(); setDropTargetPrepDate(selectedPrepDate); setDropInsertion({ prepDate: selectedPrepDate, position: event.clientY < event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2 ? index : index + 1 }); }} onDrop={(event) => receivePrepDrop(event, selectedPrepDate, event.clientY < event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2 ? index : index + 1)}>
                    <div className="instruction-line-main prep-queue-main grid-cols-[24px_minmax(0,1fr)_32px]!">
                      <label className="instruction-line-checkbox"><input type="checkbox" checked={entry.complete} disabled={disabled || entry.needsReview} aria-label={`${entry.complete ? "Reopen" : "Complete"} combined prep batch`} onChange={(event) => void mutate({ type: "setCombinedPrepStepComplete", weekId: week.id, entryId: entry.id, complete: event.target.checked })} /></label>
                      <div className="grid min-w-0 gap-[7px]">
                        <div className="flex flex-wrap items-center gap-x-2.5 gap-y-[7px]"><strong className="text-sm leading-[1.4]">{entry.instruction}</strong><span className={cn("rounded-[5px] px-2 py-[5px] text-[10px] leading-none font-bold", entry.needsReview ? "bg-[var(--amber-soft)] text-[var(--amber)]" : "bg-[var(--green-soft)] text-[var(--green-dark)]")}>{entry.needsReview ? "Needs review" : "Combined prep"}</span></div>
                        {projection.aggregates.length ? <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-[var(--ink-soft)]">{projection.aggregates.map((aggregate) => <span key={aggregate.key}>{aggregate.display}</span>)}</div> : null}
                        <div className="mt-0.5 grid max-w-[560px] gap-2 border-l-[3px] border-l-[color-mix(in_srgb,var(--green)_34%,var(--line))] py-[3px] pr-0 pl-3.5" aria-label="Source recipe steps">
                      ...
                      <details className="prep-overflow" data-prep-row-control>
                        <summary className="planner-icon-button prep-overflow-trigger inline-flex! list-none items-center! justify-center! rounded-md! border! border-[var(--line-strong)]! bg-[var(--surface)]! p-0! hover:bg-[var(--surface-muted)]! focus-visible:bg-[var(--surface-muted)]! [&::-webkit-details-marker]:hidden" aria-label={`Options for combined prep batch ${entry.instruction}`}><MoreVertical size={16} /></summary>
                        <div className="prep-overflow-menu" aria-label={`Actions for combined prep batch ${entry.instruction}`}>
                          <label className="flex min-h-8 items-center gap-[7px] px-2 py-1.5 text-[11px] text-[var(--ink-soft)] [&_input]:accent-[var(--green)]"><input type="checkbox" checked={selected} disabled={disabled} aria-label={`Select combined prep batch ${entry.instruction}`} onChange={(event) => toggleSessionEntry(entry.id, event.target.checked)} /> Select batch</label>
                          <button type="button" disabled={disabled || index === 0} onClick={(event) => { closePrepDisclosure(event.currentTarget); reorderEntry(entry.id, index, -1); }}>Move up</button>
                          <button type="button" disabled={disabled || index === selectedSession.steps.length - 1} onClick={(event) => { closePrepDisclosure(event.currentTarget); reorderEntry(entry.id, index, 1); }}>Move down</button>
                          <button type="button" disabled={disabled} onClick={(event) => { closePrepDisclosure(event.currentTarget); setCombinedInstruction(entry.instruction); setEditEntryId(entry.id); }}>Edit wording</button>
                          <button type="button" disabled={disabled} onClick={(event) => { closePrepDisclosure(event.currentTarget); if (entry.complete) previewDiscard({ type: "expandCombinedPrepStep", weekId: week.id, entryId: entry.id, discardFulfillment: true }); else void mutate({ type: "expandCombinedPrepStep", weekId: week.id, entryId: entry.id, discardFulfillment: false }); }}>Expand to separate prep steps</button>
                          <button className="danger" type="button" disabled={disabled} onClick={(event) => { closePrepDisclosure(event.currentTarget); if (entry.complete) previewDiscard({ type: "removePrepStepsFromDate", weekId: week.id, prepDate: selectedPrepDate, entryIds: [entry.id], discardFulfillment: true }); else void mutate({ type: "removePrepStepsFromDate", weekId: week.id, prepDate: selectedPrepDate, entryIds: [entry.id] }); }}>Remove</button>
                        </div>
                      </details>
                    </div>
                    {editing ? <div className="instruction-inline-comment ml-8 pt-2.5 max-[840px]:ml-0"><textarea aria-label="Combined prep instruction" value={combinedInstruction} onChange={(event) => setCombinedInstruction(event.target.value)} /><div className="step-comment-actions"><PlannerActionButton tone="secondary" type="button" onClick={() => setEditEntryId(null)}>Cancel</PlannerActionButton><PlannerActionButton tone="secondary" type="button" disabled={disabled || !combinedInstruction.trim()} onClick={() => entry.complete ? previewDiscard({ type: "updateCombinedPrepStep", weekId: week.id, entryId: entry.id, instruction: combinedInstruction.trim(), discardFulfillment: true }, "close-edit") : void mutate({ type: "updateCombinedPrepStep", weekId: week.id, entryId: entry.id, instruction: combinedInstruction.trim() }, { onAccepted: () => setEditEntryId(null) })}>Save batch</PlannerActionButton></div></div> : null}
                  </article>;
                }
                const resolved = findStep(week, entry.stepId);
                if (!resolved) return null;
                return <div key={entry.id} className="prep-queue-row-wrap group/prep-row relative" data-prep-date={selectedPrepDate} data-prep-queue-date={selectedPrepDate} data-prep-queue-position={index}>
                  {selectedSessionDropPosition === index ? <div className="prep-insertion-indicator" role="presentation" /> : null}
                  <div className="prep-queue-access-controls [@media(hover:hover)_and_(pointer:fine)]:absolute [@media(hover:hover)_and_(pointer:fine)]:top-2 [@media(hover:hover)_and_(pointer:fine)]:right-[38px] [@media(hover:hover)_and_(pointer:fine)]:z-[3] [@media(hover:hover)_and_(pointer:fine)]:border-0! [@media(hover:hover)_and_(pointer:fine)]:bg-[color-mix(in_srgb,var(--paper)_94%,transparent)]! [@media(hover:hover)_and_(pointer:fine)]:p-1! [@media(hover:hover)_and_(pointer:fine)]:opacity-0 [@media(hover:hover)_and_(pointer:fine)]:transition-opacity [@media(hover:hover)_and_(pointer:fine)]:duration-150 group-hover/prep-row:opacity-100! group-focus-within/prep-row:opacity-100!" data-prep-row-control><label><input type="checkbox" checked={selectedEntryIds.has(entry.id)} disabled={disabled} aria-label={`Select prep instruction ${resolved.step.instruction}`} onChange={(event) => toggleSessionEntry(entry.id, event.target.checked)} /> Select</label><PlannerActionButton tone="secondary" type="button" disabled={disabled || index === 0} aria-label={`Move prep instruction ${resolved.step.instruction} up`} onClick={() => reorderEntry(entry.id, index, -1)}>Up</PlannerActionButton><PlannerActionButton tone="secondary" type="button" disabled={disabled || index === selectedSession.steps.length - 1} aria-label={`Move prep instruction ${resolved.step.instruction} down`} onClick={() => reorderEntry(entry.id, index, 1)}>Down</PlannerActionButton></div>
                  <SessionStepRow
                    entry={entry}
                    prepDate={selectedPrepDate}
                    step={resolved.step}
