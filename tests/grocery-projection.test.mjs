...
test("grocery projection filters children before grouping and preserves units, literals, provenance, and unclassified children", () => {
  ...
  assert.deepEqual(produce.groups[0].quantities.map((part) => part.kind === "quantity" ? part.display : part.literal), ["1 cup sliced green onion", "1 bunch trimmed green onion"]);
...
test("shopping-relevant qualifiers remain separate even when the concept and unit match", () => {
  ...
  assert.deepEqual(groups[0].quantities.map((part) => part.literal), ["1 cup sliced green onion", "1 cup whole green onion"]);
});
...
test("grocery projection totals only compatible children while retaining package, form, and unresolved literals", () => {
  ...
  assert.deepEqual(sliced.quantities.map((part) => part.kind === "quantity" ? part.display : part.literal), ["1 cup sliced green onion", "2 cup sliced green onion", "1 package sliced green onion", "1 bunch trimmed green onion"]);
