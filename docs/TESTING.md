...
| Direct deployment | `make deploy` builds a clean `main` checkout, replaces the installed application atomically, and waits for `/api/health` plus `/api/workspace`. The production data directory is never replaced. |
