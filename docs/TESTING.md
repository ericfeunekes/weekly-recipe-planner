...
| Selected-week routing | TanStack Router is the sole location owner; root and mounted Week/Recipe/Prep/Groceries/Closeout URLs parse, validate, canonicalize, preserve exact meal occurrence identity, retain requested locations through loading/offline/reconnect, restore the last valid selected-week destination, reject the production-origin root alias, leave local controls and Codex selection outside the URL, and never synthesize planner state from a URL. Day is not a route or fallback. | Deterministic route-contract tests plus mounted phone and desktop browser coverage |
