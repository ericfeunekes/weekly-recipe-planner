...
4. **Unified capability surface.** Each planner top-level thread has hosted web search, skills, and one dynamic `planner` namespace containing exactly `read`, `preview`, `apply`, and `importRecipe`.
...
- one dynamic `planner` namespace with exactly `read`, `preview`, `apply`, and `importRecipe`;
...
- The planner namespace remains exactly `read`, `preview`, `apply`, and `importRecipe`. The internal authority manifest and command registry remain host-only.
- `read` returns bounded canonical state and versions. `preview` is pure. `apply` accepts one command or an atomic ordered batch. `importRecipe` safely reads one reviewed canonical file and atomically replaces one eligible meal. Both effect tools delegate to the same planner mutation service as the UI and Global Codex ingress.
...
| Unified capability surface | One native thread tree demonstrates worker report-to-root, a bounded user-input round trip, and dependent `planner.read`/`preview`/`apply` in one turn; generated-schema and host proof cover `planner.importRecipe`, and authenticated smoke separately demonstrates real hosted web search. No selector, intent discriminator, hidden research/planner context, worker planner namespace, or capability swap occurs. |
