...
- Install app-owned planner skills under the candidate app's `.agents/skills` tree. Production exposes that selected-app directory and its `deployment/codex/AGENTS.md` through stable symbolic links in the retained Codex home; deployment validates those links and never replaces or writes through them. Development and QA copy the links' resolved production contents into their private Codex home, so local testing cannot mutate production. Moving a tested instruction or skill update to production remains an explicit manual promotion. User-owned standalone skills are discovered dynamically rather than pinned to an application release; readiness inventories their effective names/sources and proves that adding or changing a skill cannot widen tools, RPC methods, permissions, or planner commands.
