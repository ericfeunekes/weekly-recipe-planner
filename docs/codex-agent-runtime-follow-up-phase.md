...
4. **Unified capability surface.** Each planner top-level thread has hosted web search, skills, and one dynamic `planner` namespace containing ordinary `read`, `preview`, and `apply`, plus the existing single-meal `importRecipe`. The approved-week import enters that surface only after its host/tool contract and activation gate pass.
...
- Deployment owns `deployment/codex/AGENTS.md` and configuration. The candidate app's `.agents/skills` is a direct link to the Obsidian food vault, while `CODEX_HOME/recipes` links directly to the vault recipe root. Development and QA keep their authenticated Codex home and use a fresh CWD with private copied vault skills and recipes per run, so local testing cannot mutate production or the vault. Recipe read/list/parse semantics remain host work. User-owned standalone skills are discovered dynamically rather than pinned to an application release; readiness inventories their effective names/sources and proves that adding or changing a skill cannot widen tools, RPC methods, permissions, or planner commands.
- `CODEX_HOME/AGENTS.md` and configuration are deployment-owned; food skills and recipes are Obsidian-owned inputs. Loading those declared sources does not create a general-purpose filesystem capability.
...
- standalone skills from the normal `$HOME/.agents/skills` surface plus any source-bound food skills enabled;
- one dynamic `planner` namespace with ordinary `read`, `preview`, and `apply`, plus the existing single-meal `importRecipe`; approved-week import is unavailable until its host/tool contract and activation gate pass;
...
No request field, UI mode, prompt classifier, or research/planner child-session transition swaps the top-level capabilities. The owning top-level agent may interleave planner tools, hosted search, and worker delegation in the order the task requires. Workers may use their empirically proven research/reasoning surface and return results to the parent, but the current app-server starts them with an empty dynamic-tool vector. The parent remains the only embedded caller of the planner namespace.
...
That behavioral coverage may be packaged as one or a small set of automatically discoverable planner skills. Packaging is not a user-visible mode and does not change the fixed tool surface. Readiness proves that declared source-bound food skills reach the model, not merely that files exist, and that user-owned standalone skills remain dynamically discoverable without becoming release or capability authority.
...
- The planner namespace currently contains ordinary `read`, `preview`, and `apply`, plus the existing single-meal `importRecipe`. The internal authority manifest and command registry remain host-only. The approved-week import remains unavailable until its host/tool contract and activation gate pass; neither skills nor agents emulate it with repeated imports.
- `read` returns bounded canonical state and versions. `preview` is pure. `apply` accepts one command or an atomic ordered batch. `importRecipe` safely reads one reviewed canonical file and atomically replaces one eligible meal. When activated, the approved-week operation validates the exact map of existing meal occurrence IDs to root-relative canonical paths and reviewed revisions, requires targeted meals plus explicitly untargeted unsourced manual meals to exhaust the week shell, preserves meal IDs, builds the complete candidate week, and commits all targets or none through the same mutation service as the UI and Global Codex ingress. An untargeted manual meal with `sourceRecipe` or a canonical source pin rejects. Missing, duplicate, unknown, stale, or revision-mismatched targets reject without mutation. Its accepted response is authoritative; normal workflow adds no receipt, manual readback, or mandatory UI inspection.
...
- The no-auth disposable provider probe exercises one native root through worker spawn, exact bounded `wait_agent` mailbox completion, the worker's final report returned to the root, a bounded `request_user_input` answer, and dependent `planner.read`/`preview`/`apply` callbacks. It verifies `importRecipe` through generated-schema, compatibility, and host-boundary tests rather than mutating a meal during the no-auth capability probe. It also proves the exact `approvalPolicy:"never"` readback and rejects any approval request emitted by negative fixtures. Generated-schema checks and backend protocol tests prove the method-specific command/file/permission/MCP decline responses and sanitized notices, because the capability-closed positive probe cannot honestly induce forbidden tools. Authenticated live smoke remains separately responsible for real ChatGPT, hosted web-search, and end-to-end household behavior; it does not replace these deterministic protocol negatives.
...
- The authenticated dedicated home is retained across application stage, activation, rollback, and recovery. Release operations may deploy configuration, AGENTS, and the Obsidian food links, but may not wipe, replace, roll back, or silently migrate native thread history. User-owned standalone skill content remains outside the application release identity.
...
| Unified capability surface | One native thread tree demonstrates worker report-to-root, a bounded user-input round trip, and dependent `planner.read`/`preview`/`apply` in one turn; generated-schema and host proof cover single-meal import. The all-or-nothing approved-week operation is absent until its own host/tool activation proof passes, then joins this cell. Authenticated smoke separately demonstrates real hosted web search. No selector, intent discriminator, hidden research/planner context, worker planner namespace, or capability swap occurs. |
| Skill discovery and provenance | Obsidian-owned food skills are source-bound through managed production links or private dev/QA copies; normal `$HOME/.agents/skills` entries remain dynamically discoverable; captured model input/readback plus live behavior proves skills reach the top-level agent. Any skill guidance observed in a worker is inventoried rather than assumed. Adversarial or changed skills cannot widen tools, grants, RPC, or planner commands. |
...
| Planner operation parity | UI, the embedded top-level agent, and Global Codex use the same typed command registry and mutation authority. When activated, approved-week import shares conflict, replay, event, and authoritative-result semantics without an alternate kernel. Workers have no direct planner ingress. |
...
| Release retention | Stage/activation/rollback preserves the authenticated agent home and native catalogue, revalidates the selected pointer, deploys configuration/instruction sources and validates exact food links without pinning user skills, excludes synthetic probes from household history/production data, and binds a fresh candidate to its own evidence. |
...
- **AC-2 — Natural capability use:** the **Unified capability surface** and **Web-assisted planning** cells in [TESTING.md](TESTING.md) prove the agent can choose and interleave planner skills, web search, and the planner namespace without a public intent.
...
- **AC-5 — Skills without capability expansion:** the **Skill discovery and provenance** cell proves source-bound food and normal standalone skills reach the top-level agent without adding capabilities or archive authority; any worker skill guidance is accepted only when the active runtime proves it and still cannot add planner tools.
