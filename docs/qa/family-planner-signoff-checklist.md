...
| PREP-12 | P0 | Apply the combined entry, then complete and reopen it. | One prep-owned row exposes both contributions and their ingredient literals; its fulfillment changes without changing either canonical step completion. | Prep/Day screenshots and authoritative readback | NOT RUN |
...
| GROC-07 | P0 | Start with a Needs source requirement, classify it as Farm box or On hand, filter it, and open its linked dinner. | Source updates once, To buy contains only Shop items, the requirement remains visible in its coverage filter, and the dinner link opens the correct recipe. | Before/after screenshot, authority readback, and recipe drawer recording | NOT RUN |
