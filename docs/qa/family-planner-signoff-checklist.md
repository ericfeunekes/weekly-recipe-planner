...
| NAV-13 | P0 | At the development root and mounted `/recipe-planner/` base path, direct-load/reload/copy Week, Recipe, Prep, Groceries, and Closeout; test malformed, missing, archived-ineligible, and cross-week meal IDs. | TanStack Router is the sole browser-location owner. Valid URLs retain the exact destination and meal identity; an invalid Recipe returns to its requested valid Week with a clear unavailable-recipe state and never opens another meal. Day is neither a route nor a fallback. | Root and mounted URLs, browser recording, route/location assertions, and accessibility scan | NOT RUN |
