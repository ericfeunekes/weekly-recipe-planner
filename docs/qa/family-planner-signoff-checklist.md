...
| Dinner cook | Opens the exact Recipe, sees completed prep, and finishes the meal |
...
| D11 | Disposable installed layout under a temporary `HOME`, with fixed `app` and `app.previous` slots plus stable data and agent roots |
...
| ID | Priority | User story and actions | Expected result | Evidence | Status |
| --- | --- | --- | --- | --- | --- |
| NAV-01 | P0 | Navigate Week, Recipe, Prep, Groceries, and Close out on desktop. | Each control selects one view, updates `aria-current`, focuses the H1, and resets scroll to the top. | Recording and focus/scroll readback | NOT RUN |
...
| NAV-08 | P1 | Select a week that does not contain today and open a valid Recipe. | The exact selected meal opens regardless of whether its Week contains today. Unavailable-recipe state is reserved for malformed, missing, archived-ineligible, or cross-week targets and never renders another meal. | Screenshot at all core viewports and meal-ID assertion | NOT RUN |
| NAV-09 | P0 | Open the second and third populated meal cards on one date, copy each `/weeks/<weekId>/recipes/<mealId>` URL, reload, and use back/forward. | Each URL opens its exact meal. No Day route or first-meal-by-date fallback is used. | URLs, recording, and meal IDs | NOT RUN |
| IMPORT-01 | P0 | Before approved-week host/tool/compatibility activation, request the approved-week import. | The operation is explicitly unavailable; the workflow stops without emulating it through repeated single-meal imports, a receipt, routine readback, or mandatory UI inspection. | Tool trace and workflow recording | NOT RUN |
| NAV-13 | P0 | At the development root and mounted `/recipe-planner/` base path, direct-load/reload/copy a valid Week/Recipe URL; test malformed, missing, archived-ineligible, and cross-week meal IDs; and open legacy Day URLs. | TanStack Router is the sole browser-location owner. Valid URLs retain the exact Week/meal; an invalid Recipe returns to its requested valid Week with a clear unavailable-recipe state and never opens another meal; a legacy Day URL returns to its matching Week date context without selecting a meal. | Root and mounted URLs, browser recording, route/location assertions, and accessibility scan | NOT RUN |
| NAV-14 | P0 | Start on a valid Week/Recipe URL, then load, go offline, reconnect, use copied-tab load and back/forward, and open the application root with and without a remembered valid selected-week destination; verify the production-origin root alias. | The requested location remains in history while authority is unavailable and is validated only after reconnect; root restores the last valid selected-week destination or the authoritative default and canonicalizes its address; the production-origin root alias rejects. | Root and mounted recordings, history/location trace, authoritative workspace trace, and accessibility scan | NOT RUN |
...
| MEAL-01 | P0 | Edit title, venue, subtitle, ingredients, notes, prep note, and leftover note; save. | One shared meal snapshot persists and appears in Week, Recipe, and the reopened drawer. | Before/after screenshots and API readback | NOT RUN |
...
| MEAL-04 | P0 | Move a dinner to an open date. | Week and Recipe update atomically; event history identifies the move. | Before/after week screenshots | NOT RUN |
...
| MEAL-07 | P0 | Start cooking from Recipe and then mark cooked. | Status persists across clients and reload; leftover availability follows the canonical rule once, and the resulting culinary state is frozen as the dated cooked version. | Two-client recording and readback | NOT RUN |
| MEAL-15 | P0 | After approved-week host/tool/compatibility activation, import a complete approved week with targeted recipe meals and explicitly untargeted manual meals. | Existing meal IDs map to root-relative canonical paths and exact reviewed revisions; targets plus unsourced manual meals exhaust the week shell and preserve meal IDs. An untargeted manual meal with `sourceRecipe` or a canonical source pin rejects. Missing, duplicate, unknown, stale, or revision-mismatched targets reject with no mutation. | Transaction/replay proof plus one visible week/grocery/recipe journey | NOT APPLICABLE |
| MEAL-16 | P0 | After approved-week host/tool/compatibility activation, run the meal-planning approved-week workflow once, then exercise rejected responses. | The workflow trusts one accepted import response and proceeds; rejection stops visibly without per-meal retries, receipt/readback ceremony, or mandatory UI inspection. | Tool trace and workflow recording | NOT APPLICABLE |
| MEAL-17 | P0 | After approved-week host/tool/compatibility activation, explicitly edit one imported meal before cooking, restart and undo, then mark it cooked and attempt another culinary edit plus non-culinary updates. | The pre-cook edit preserves source lineage, updates groceries, and survives restart/undo. Cooking anchors the culinary state to its event/version and feedback; the post-cook culinary edit rejects while concept resolution, grocery coverage/check state, applicable instruction completion and running timer state, Prep organization, logistics, and feedback remain mutable. No implicit edit entered import; later reusable promotion is an explicit separately reviewed food-workflow action. | Transaction/event/feedback proof plus one visible recipe/grocery journey | NOT APPLICABLE |
...
| ID | Priority | User story and actions | Expected result | Evidence | Status |
| --- | --- | --- | --- | --- | --- |
| ING-01 | P0 | Add one similar ingredient, then paste a mixed list containing exact household labels, accepted vocabulary, similar names, and a novel ingredient. | The single literal is added immediately and offers a reversible in-place correction; batch preview returns one stable result per input with direct matches, explainable alternatives, and unresolved/new-concept choices while changing no planner state. | Add response, batch preview payload, ingredient editor screenshots, and workspace readback | NOT RUN |
| ING-02 | P0 | Keep an ambiguous ingredient unresolved, then cook it and add it to an instruction step. | Recipe and Prep retain the literal ingredient and remain operable; only canonical grouping remains unresolved. | Cross-view screenshots and ingredient/step identity readback | NOT RUN |
...
| ING-05 | P0 | Include raw ingredients, a prepared tray, cooked/reserved portions, and a finished dish across recipe steps; leave one raw requirement without a source. | Every weekly requirement projects exactly once; prepared outputs, leftovers, and finished dishes do not project; the unclassified requirement says Needs source instead of appearing in Shop. | Recipe/step inputs, grocery screenshot, and authority readback | NOT RUN |
...
| ING-F03 | P0 | Preview an ordered mixed ingredient batch, change either its literal input order or the catalogue, then attempt the original apply. | Preview is pure and returns one correlated result per line. Apply accepts only the exact digest, planner version, and catalogue revision; stale or reordered input produces no partial mutation. | `tests/ingredient-resolution-lifecycle.test.mjs`; `tests/integration/ingredient-occurrence-ingress-parity.test.mjs` | PASS (AUTOMATED) |
| ING-F06 | P0 | Rename and merge concepts used by active and archived recipes, undo the latest change, restart, and read through browser HTTP, embedded Codex, and Global Codex. | All three entry points expose the same catalogue and lifecycle result. Occurrence IDs and literal source, amount, unit, ingredient, and qualifier fields remain byte-for-byte stable through apply, undo, and restart. | Composed three-ingress lifecycle in `tests/integration/ingredient-occurrence-ingress-parity.test.mjs`; active/archived literal coverage in `tests/ingredient-resolution-lifecycle.test.mjs`; schema restart coverage in `tests/store-schema-v10-activation.test.mjs` | PASS (AUTOMATED) |

### Instruction Steps And Timers

| ID | Priority | User story and actions | Expected result | Evidence | Status |
| --- | --- | --- | --- | --- | --- |
| STEP-01 | P0 | Complete a step from Recipe. | The same canonical step is checked in Meal and Prep immediately and after reload. | Cross-view and reload recording | NOT RUN |
| STEP-02 | P0 | Reopen a completed step from Prep. | Completion clears in Recipe and Meal without changing prep order or instruction order. | Cross-view screenshots | NOT RUN |
...
| PREP-07 | P0 | Complete, note, and time a step from Prep; use its recipe link to edit instruction content in the Meal drawer. | Every action updates the canonical step and projects to Prep, Recipe, and Meal. | Cross-view recording | NOT RUN |
| PREP-08 | P0 | Follow the recipe link from a long mobile prep list. | Touch target is at least 44px, opens the linked exact meal's canonical Recipe location, and returning preserves scroll/order. | Phone recording, URL, meal ID, and bounding box | NOT RUN |
...
| PREP-11 | P0 | Select two direct references from different recipes and preview combining them with one prep-specific instruction. | The preview names the date, queue position, both contributing recipes/steps, ingredient occurrences, and the two-to-one replacement before mutation. | Preview screenshot and unchanged-state readback | NOT RUN |
| PREP-12 | P0 | Apply the combined entry, then complete and reopen it. | One prep-owned row exposes both contributions and their ingredient literals; its fulfillment changes without changing either canonical step completion. | Prep/Recipe screenshots and authoritative readback | NOT RUN |
| PREP-13 | P0 | Edit the combined wording, move it, reload, expand it, and undo the expansion. | Wording and fulfillment survive restart; expansion restores one direct reference per source in contribution order; undo restores the combined entry without recipe mutation. | Event/history, restart readback, and before/after screenshots | NOT RUN |
| PREP-13A | P0 | Complete a combined entry, then edit one contributing instruction or linked ingredient. | The combined entry reopens as needing review, every prepared badge disappears, and reconfirmation is required before it can be completed again. | Before/after projection, event, and recipe screenshots | NOT RUN |
| PREP-13B | P0 | Edit, remove, expand, or clear a date containing a completed combined entry. | The unconfirmed operation is rejected; confirmed preview enumerates every fulfillment that will be discarded; canonical steps remain unchecked; undo restores the prior combined entry. | Preview, event/readback, and before/after screenshots | NOT RUN |
| PREP-14 | P0 | Combine compatible and incompatible ingredient amounts, including a raw-to-cooked form. | Safe compatible totals may derive; every unsupported or incompatible source remains visibly literal; no aggregate becomes copied recipe or occurrence truth. | Ingredient-lineage readback and Prep screenshot | NOT RUN |
| PREP-15 | P0 | Create the same valid combined entry through UI, embedded Codex, and Global Codex preview/apply paths on independent disposable fixtures; also submit duplicate, missing, cross-week, and stale-version sources. | All three ingresses expose equivalent lineage and accepted state; invalid inputs reject atomically with no orphaned references or recipe changes. | Contract responses, event/undo readback, and browser projection | NOT RUN |
...
| GROC-06 | P1 | View same-concept requirements with different literal amounts, sources, completion states, or incompatible measures. | Compatible amounts total automatically; incompatible measures/forms remain separately visible. Stable child identities keep source/check actions attached to the intended requirements. | Screenshot and command readback | NOT RUN |
| GROC-07 | P0 | Start with a Needs source requirement, classify it as Farm box or On hand, filter it, and open its linked recipe. | Source updates once, To buy contains only Shop items, the requirement remains visible in its coverage filter, and the recipe link opens the exact meal. | Before/after screenshot, authority readback, and recipe recording | NOT RUN |
...
| LEFT-03 | P0 | Assign available leftovers to an open dinner. | The Week and Recipe surfaces show the assigned leftover instead of a meal. | Cross-view screenshots | NOT RUN |
...
| ID | Priority | User story and actions | Expected result | Evidence | Status |
| --- | --- | --- | --- | --- | --- |
| CHAT-01 | P0 | Select one native top-level thread, then send an ordinary message from each primary planner view. | Every view uses the selected thread and carries correct week/view and exact Recipe meal/leftover context. Navigation creates no new thread and arbitrary selected-object context is not falsely claimed. | Selected pointer, native thread/item, and context readback | NOT RUN |
...
| DEPLOY-01 | P0 | Run `make promote` from a dirty checkout whose committed local `main` is known. Fail dependency install, mounted-path tests, lint, and schema compatibility in turn. | Only committed `main` enters the detached candidate; every gate failure occurs before service disturbance or slot rotation. | Gate output, selected-slot listing, service readiness, database `quick_check` | NOT RUN |
| DEPLOY-02 | P0 | Using D11, terminate promotion before and during each slot rename, service bootstrap, and readiness check; then run `make recover`. | Recovery selects only `app` or `app.previous`, leaves one ready service when either slot is usable, and never migrates, copies, restores, or prunes SQLite. | Directory listing, service health/workspace, database before/after readback | NOT RUN |
| DEPLOY-03 | P0 | After a candidate becomes ready, make new planner/selection/tool writes, force service failure, and run `make recover` repeatedly. | Recovery never rolls back the database or dedicated agent home. It preserves a ready current app or selects the immediate previous app, with the newer writes still readable. | Before/after workspace, native selection, slot listing, repeated recovery output | NOT RUN |
| DEPLOY-04 | P0 | Promote twice in a disposable installed layout, verify the mounted browser, force service failure, recover, restart, and recover again. | Only `app` and `app.previous` remain; both shipped commands are repeatable; mounted health, workspace, assets, favicon, and browser interaction work after recovery. | Installed screenshot, HTTP readback, slot listing, service log | NOT RUN |
| DEPLOY-05 | P0 | Run the disposable promotion with compatible authenticated Codex and Global Codex ingress, then inject each readiness dependency failure. | Candidate selection requires causal fresh-connection readiness; no probe reads or writes the household database, LaunchAgent, or dedicated agent home. | Disposable profile, health responses, household-target refusal tests | NOT RUN |
...
| Recipe | Outside week / no meal | NOT APPLICABLE: no per-view loader | Offline/pending mutation | Recipe states plus D8 assigned leftovers | D4 recipe/steps/source | Status, checkbox, note, timer, meal drawer |
...
- All P0 stories applicable to the candidate's activated capability surface are `PASS`, or each failure is fixed and re-run before signoff. Before approved-week activation, `MEAL-15` through `MEAL-17` are `NOT APPLICABLE` and `IMPORT-01` is the P0 approved-week boundary; on and after activation, `MEAL-15` through `MEAL-17` become applicable.
...
- Database `quick_check`, planner receipt/event/effect counts, fixed-slot listings, and before/after workspace readback accompany durability and deployment claims; releases add no identity manifest or receipt layer.
