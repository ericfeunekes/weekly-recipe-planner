...
**Status:** explicitly deferred from this implementation and signoff on 2026-07-10; the architecture is finalized in `docs/codex-agent-runtime-follow-up-phase.md`, and its implementation-shaping register is closed in `docs/codex-agent-runtime-follow-up-unknowns.md`.
...
The follow-up intentionally extends the durable chat lifecycle: several dependent tool calls may commit separately before the terminal reply, and each accepted effect must be fenced, recorded, idempotent, and visible even if the reply later fails. That extension is not a missing current proof cell and must not be pulled into this implementation.

Family readiness is one dependency-gated parallel release train. The Codex-runtime follow-up is a genuinely separate release: it consumes the stable state/mutation/chat service, passes its own capability-isolation gate, proves the extended tool/effect lifecycle in an opt-in harness, and then atomically replaces the bounded adapter. Building that runtime first would target browser-owned state and command contracts that this plan is replacing.
